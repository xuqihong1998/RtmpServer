...
RtmpConnection::RtmpConnection(std::shared_ptr<RtmpServer> server, muduo::net::TcpConnectionPtr conn)
    :RtmpConnection(conn, server.get())
{
...
/*
RtmpConnection::RtmpConnection(muduo::net::EventLoop* loop, int sockfd, Rtmp* rtmp)
    ,poll_(new muduo::net::EPollPoller(loop))
    ,channel_(new muduo::net::Channel(loop, sockfd))
{
    ...
    this->setMessageCallback([this](std::shared_ptr<TcpConnection>& conn, muduo::net::Buffer* buffer, muduo::Timestamp& timestamp)
    {
...
RtmpConnection::RtmpConnection(muduo::net::TcpConnectionPtr conn, Rtmp* rtmp)
{
    rtmp_chunk_ = std::make_shared<RtmpChunk>();
    conn_ = conn;
    connection_state_ = HANDSHAKE;
    peer_bandwidth_ = rtmp->GetPeerBandwidth();
    acknowledgement_size_ = rtmp->GetAcknowledgementSize();
...
bool RtmpConnection::SendMediaData(uint8_t type, uint64_t timestamp, std::shared_ptr<char> payload, uint32_t payload_size)
{
    ...
    //获取TcpConnectionPtr 对应的eventloop
    muduo::net::EventLoop* event_loop = conn_->getLoop();
	event_loop->runInLoop([conn, type, timestamp, payload, payload_size] {
...
bool RtmpConnection::SendVideoData(uint64_t timestamp, std::shared_ptr<char> payload, uint32_t payload_size)
{
    ...
    //获取TcpConnectionPtr 对应的eventloop
    muduo::net::EventLoop* event_loop = conn_->getLoop();
	event_loop->runInLoop([conn, timestamp, payload, payload_size] {
		RtmpMessage rtmp_msg;
		rtmp_msg.type_id = RTMP_VIDEO;
		rtmp_msg._timestamp = timestamp;
		rtmp_msg.stream_id = conn->stream_id_;
