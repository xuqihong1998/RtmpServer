...
int RtmpChunk::Parse(muduo::net::Buffer& in_buffer, RtmpMessage& out_rtmp_msg)
{
    int ret = 0;
    if(0 == in_buffer.readableBytes())
    {
        return 0;
    }
...
int RtmpChunk::ParseChunkHeader(muduo::net::Buffer& buffer)
{
    ...
    */
    ...
    uint8_t fmt = (flags >> 6); // get message header
    if(fmt >= 4)
    {
        return -1;
    }

    uint32_t header_len = kChunkMessageHeaderLen[fmt];
...
int RtmpChunk::ParseChunkBody(muduo::net::Buffer& buffer)
{
    uint32_t byte_used = 0;
    uint8_t* buf = (uint8_t*)buffer.peek();
    uint32_t buf_size = buffer.readableBytes();
    ...
    if(buf_size < (chunk_size + byte_used))
    {
        return 0;
    }
    ...
    memcpy(rtmp_msg.payload.get() + rtmp_msg.index, buf + byte_used, chunk_size);
    byte_used += chunk_size;
